class VoiceChatClient {
    constructor() {
        ...
        this.sidebarBackBtn = document.querySelector('.sidebar-header .back-btn'); // Новый селектор
        ...
        const handleBackClick = () => {
            ...
            this.hideUI(); // Скрываем UI и отправляем сообщение
        };
        if (this.backBtn) {
    ...
    setupInitialUI() {
        ...
        // Изначально кнопка микрофона отключена и показывает состояние подключения
        this.micButton.disabled = true;
    ...
    async ensureStream() {
        ...
        // Проверяем, есть ли поток и активен ли в нем аудио трек
    ...
    async connectToServer() {
        ...
        // Проверка, если уже подключен или подключение в процессе
        if (this.isConnected) {
        ...
        this.isConnectionInProgress = true; // Устанавливаем флаг
        try {
            ...
            if (this.compactMicBtn) {
                 this.compactMicBtn.disabled = true;
                 this.compactMicBtn.classList.remove('active'); // Сбрасываем состояние
            }
            ...
            this.isConnectionInProgress = false; // Сбрасываем флаг
            this.updateStatus('Подключено к серверу', 'normal');
            // Теперь кнопка микрофона активна
            this.micButton.disabled = false;
            this.micButtonText.textContent = 'Включить микрофон';
            if (this.compactMicBtn) {
                 this.compactMicBtn.disabled = false;
                 this.compactMicBtn.title = 'Включить/выключить микрофон'; // Добавляем подсказку
            }
            const toggleMicHandler = () => {
                console.log('[MIC] Нажата кнопка микрофона');
                this.toggleMicrophone();
            };
            this.micButton.onclick = toggleMicHandler;
            if (this.compactMicBtn) {
                this.compactMicBtn.onclick = toggleMicHandler;
            }
            ...
            this.isConnectionInProgress = false; // Сбрасываем флаг при ошибке
            ...
            this.micButton.disabled = false; // Разблокируем кнопку
            this.micButtonText.textContent = 'Ошибка подключения';
            if (this.compactMicBtn) {
                 this.compactMicBtn.disabled = false;
                 this.compactMicBtn.title = 'Ошибка подключения'; // Добавляем подсказку об ошибке
            }
            this.addMessage('System', 'Ошибка подключения: ' + errorMessage);
            // Сброс кнопки через 3 секунды
            setTimeout(() => {
                 // Проверяем, не подключились ли мы в это время (например, повторная попытка)
                 if (!this.isConnected) {
                    console.log('[CONNECT] Сброс кнопки после ошибки');
                    this.micButtonText.textContent = 'Включить микрофон'; // Или 'Повторить подключение'?
                    const retryHandler = () => {
                        console.log('[UI] Нажата кнопка повторного подключения');
                        this.connectToServer(); // Позволяем повторить попытку
                    };
                    this.micButton.onclick = retryHandler;
                    if (this.compactMicBtn) {
                         this.compactMicBtn.onclick = retryHandler;
                         this.compactMicBtn.title = 'Повторить подключение'; // Обновляем подсказку
                    }
                }
            }, 3000);
        }
    ...
    startKeepAlive() {
        ...
        this.keepAliveInterval = setInterval(async () => {
            try {
                // console.log('[KEEPALIVE] Отправка keep-alive');
                await this.registerClient();
                // console.log('[KEEPALIVE] Keep-alive отправлен');
            } catch (error) {
                console.log('[KEEP-ALIVE ERROR]', error);
                // this.updateStatus('Проблемы с соединением', 'disconnected');
            }
        }, 5000);
    }
    ...
    async startMicrophone() {
        ...
        try {
            ...
            this.addMessage('System', 'Микрофон включен - вас слышат!');
    ...
    async stopMicrophone() {
        ...
        try {
            ...
            if (this.stream) {
                ...
                this.stream = null; // Сбрасываем ссылку на поток
            }
            this.isMicActive = false;
            this.micButton.classList.remove('active');
            this.micButtonText.textContent = 'Включить микрофон';
    ...
    async updateParticipants() {
        try {
            ...
            // Получаем аудио только от других участников
    ...
    updateMembersList(clients) {
        // Фильтруем список, исключая себя
    ...
    async consumeClientProducers(clientId) {
        try {
            // Не создаем consumer для самого себя!
    ...
    async consumeProducer(producerId, clientId) {
        try {
            ...
            // Создаем скрытый аудио элемент для воспроизведения
                ...
                if (playPromise !== undefined) {
                    playPromise
                        ...
                        .catch(error => {
                            ...
                            // Попробуем изменить настройки аудио
    ...
    async startParticipantUpdates() {
        ...
        await this.updateParticipants(); // Первый вызов
    ...
    /**
     * Скрывает UI голосового чата и отправляет сообщение родительскому окну для закрытия iframe.
     */
    hideUI() {
        console.log('[UI] Скрытие интерфейса голосового чата');
        try {
            // Предположим, у нас есть контейнер для всего UI чата
            const appContainer = document.querySelector('.app');
            const statusBar = document.querySelector('.status-bar'); // Предполагаемый селектор
            if (appContainer) {
            ...
            // Отправляем сообщение родительскому окну для закрытия iframe
            if (window.parent) {
                console.log('[UI] Отправка сообщения родителю для закрытия голосового чата.');
                window.parent.postMessage({ type: 'CLOSE_VOICE_CHAT' }, '*'); // Используйте конкретный origin для большей безопасности
            } else {
                console.warn('[UI] window.parent не доступен. Не удалось отправить сообщение для закрытия.');
                // Резервный вариант: вызов Electron API, если оно доступно напрямую в iframe (менее вероятно)
    ...
    /**
     * Показывает UI голосового чата, если он был скрыт с помощью hideUI.
     * Предполагает, что DOM-элементы не были удалены.
     */
    showUI() {
        ...
        try {
            ...
            if (appContainer) {
                appContainer.style.display = ''; // Сбрасываем стиль display
            } else {
    ...
    destroy() {
        ...
        this.hideUI(); // Сначала скрываем UI
        // Остальная логика освобождения ресурсов
        if (this.keepAliveInterval) {
            clearInterval(this.keepAliveInterval);
            this.keepAliveInterval = null; // Хорошая практика
        }
        if (this.updateInterval) {
            clearInterval(this.updateInterval);
            this.updateInterval = null;
        ...
        this.consumers.clear(); // Очищаем Map
        ...
        if (window.audioElements) {
            window.audioElements.forEach((element, key) => {
                if (element && element.parentNode) {
                    element.pause();
                    element.srcObject = null; // Освобождаем поток
                    element.parentNode.removeChild(element);
                }
            });
            window.audioElements.clear(); // Очищаем Map
        }
        this.isConnected = false;
        this.isMicActive = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Проверяем, загружена ли mediasoupClient
    if (typeof mediasoupClient === 'undefined') {
    ...
    if (voiceChatClient) {
         voiceChatClient.connectToServer().catch(err => {
             ...
             // Сообщение об ошибке уже будет показано в connectToServer
         });
    }
    ...
    const checkScreenSizeAndAdjustUI = () => {
        if (window.innerWidth <= 900) {
            // Компактный режим
            if (compactMicBtn) {
                compactMicBtn.style.display = 'flex'; // Показываем кнопку
            }
        } else {
            // Развернутый режим
            if (compactMicBtn) {
                compactMicBtn.style.display = 'none'; // Скрываем кнопку
            }
        }
    };
    // Проверяем при загрузке
    checkScreenSizeAndAdjustUI();
    // И проверяем при изменении размера окна
    ...
    document.addEventListener('click', (e) => {
        if (isSidebarVisible && !sidebar.contains(e.target) && e.target !== toggleSidebarBtn) {
            isSidebarVisible = false;
            sidebar.classList.remove('visible');
        }
        if (isMembersPanelVisible && !membersPanel.contains(e.target) && e.target !== toggleMembersBtn) {
