...
class AddonManager {
    ...
    // Загрузка конфигурации с fallback по всем источникам
    async _fetchConfigWithFallback() {
        const total = CONFIG_URLS.length;

        for (let i = 0; i < total; i++) {
            const url = CONFIG_URLS[i];
            this._emitEvent(this.mainWindow, 'config-source-status', { index: i + 1, total, url, state: 'trying' });
            try {
                logger.info(`[CONFIG] Trying to load from: ${url}`);
                ...
                if (!config.addons) {
                    logger.warn(`[CONFIG] No 'addons' field in response from ${url}`);
                    this._emitEvent(this.mainWindow, 'config-source-status', { index: i + 1, total, url, state: 'failed' });
                    continue;
                }
                logger.info(`[CONFIG] Successfully loaded from: ${url}`);
                this._emitEvent(this.mainWindow, 'config-source-status', { index: i + 1, total, url, state: 'loaded' });
                return config;
            } catch (error) {
                logger.warn(`[CONFIG] Failed to load from ${url}: ${error.message || error}`);
                this._emitEvent(this.mainWindow, 'config-source-status', { index: i + 1, total, url, state: 'failed' });
            }
        }
    ...
    async _forceReinstall(mainWindow) {
        ...
        for (const addonName of addonsToReinstall) {
            ...
            try {
                ...
                // Проверяем что аддон действительно установился
                const gamePath = this.getGamePath();
                const isActuallyInstalled = this._checkInstalled(addonName, addon.target_path, gamePath);
                
                if (isActuallyInstalled) {
                    logger.info(`[REINSTALL] ${addonName} installed successfully, waiting 3 seconds...`);
                    await new Promise(r => setTimeout(r, 3000));
                } else {
                    logger.warn(`[REINSTALL] ${addonName} installation verification failed`);
                }
    ...
    async toggleAddon(name, install, mainWindow) {
        ...
        try {
            if (install) {
                ...
                // Проверяем что аддон действительно установился
                const gamePath = this.getGamePath();
                const isActuallyInstalled = this._checkInstalled(name, addon.target_path, gamePath);
                
                if (isActuallyInstalled) {
                    logger.info(`[TOGGLE] ${name} installed successfully, waiting 3 seconds for full initialization...`);
                    await new Promise(r => setTimeout(r, 3000));
                } else {
                    logger.warn(`[TOGGLE] ${name} installation verification failed`);
    ...
    async _autoUpdateNSQC3(mainWindow) {
        ...
        // Проверяем установку NSQC3
        const isActuallyInstalled = this._checkInstalled('NSQC3', nsqc3.target_path, gamePath);
        if (isActuallyInstalled) {
            logger.info('[AUTO_UPDATE] NSQC3 installed successfully, waiting 3 seconds...');
            await new Promise(r => setTimeout(r, 3000));
        }
    }
